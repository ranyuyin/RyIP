# -*- coding: utf-8 -*-
"""
Created on Mon Mar 06 14:18:44 2017
...
def pix2map(dataset, pixel, line):
    '''
    ...
    px = trans[0] + pixel * trans[1] + line * trans[2]
    py = trans[3] + pixel * trans[4] + line * trans[5]
    return px, py


def map2pix(dataset, x, y):
    '''
    根据GDAL的六 参数模型将给定的投影或地理坐标转为影像图上坐标（行列号）
    :param dataset: GDAL地理数据
    :param x: 投影或地理坐标x
    :param y: 投影或地理坐标y
    :return: 影坐标或地理坐标(x, y)对应的影像图上行列号(pixel, line)
    '''
    trans = dataset.GetGeoTransform()
